...
def libraryScan(dir=None, append=False, ArtistID=None, ArtistName=None, cron=False):
    ...
    media_exts = ['.' + x.lower() for x in headphones.MEDIA_FORMATS]
    ...
    for r,d,f in os.walk(dir):
        ...
        for directory in d[:]:
            if directory.startswith("."):
                d.remove(directory)
        for files in f:
            if any(files.lower().endswith(ext) for ext in media_exts):
    ...
    for song in song_list:
        ...
        # Check if we know about song location
        existing_song = myDB.action('SELECT (SELECT count(*) FROM tracks WHERE Location=?) + (SELECT count(*) FROM tracks WHERE Location=?) as cnt', [song['Location'], song['Location']]).fetchone()
        if existing_song['cnt'] != 0:
            # We already know about that song, skip to the next one
            continue
        ...
        if song['ReleaseID'] and song['TrackTitle']:
            ...
            track = myDB.action('SELECT TrackID, AlbumID from alltracks WHERE ReleaseID=? AND TrackTitle=?', [song['ReleaseID'], song['TrackTitle']]).fetchone()

            if not track:
                track = myDB.action('SELECT TrackID, AlbumID from tracks WHERE ReleaseID=? AND TrackTitle=?', [song['ReleaseID'], song['TrackTitle']]).fetchone()
        ...
        if song['TrackID'] and song['AlbumTitle']:
            ...
            track = myDB.action('SELECT AlbumTitle from alltracks WHERE TrackID=? AND AlbumTitle LIKE ?', [song['TrackID'], song['AlbumTitle']]).fetchone()

            if not track:
                track = myDB.action('SELECT AlbumTitle from tracks WHERE TrackID=? AND AlbumTitle LIKE ?', [song['TrackID'], song['AlbumTitle']]).fetchone()
        ...
        if song['TrackID']:
            ...
            track = myDB.action('SELECT count(*) as cnt from alltracks WHERE TrackID=?', [song['TrackID']]).fetchone()

            if not track['cnt']:
                track = myDB.action('SELECT count(*) as cnt from tracks WHERE TrackID=?', [song['TrackID']]).fetchone()
