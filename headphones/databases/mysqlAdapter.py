...
class DBConnection:
    ...
    def __init__(self):
        ...
        if sqlConnection == None:
            try:
                ...
                logger.error('Database error in init: %s' % e)
            except Exception as ex:
                logger.error('Error during database connection: %s' % ex)
        else:
    ...
    def query(self,cursor,sql,args=None):
    ...
    def tableExport(self,tableName):
        ...
        rows.execute("SELECT * from "+tableName)
    ...
    def tableCopyFrom(self,sc,tableName):
    ...
    def selectOne(self,sql,args=None):
        ...
        try:
            sql = sql + " LIMIT 1"
            c = self.getCursor()
            self.query(c,sql,args)
            sqlResults = c.fetchone()
            if sqlResults <> None: sqlResults = dict(zip(c.column_names,sqlResults))
            c.close()
        except mysql.connector.Error as e:
    ...
    def select(self, sql, args=None):    
        ...
        try:
            ...
            if sqlResults <> None: sqlResults = [ dict(zip(c.column_names,row)) for row in sqlResults ]
            c.close()
        except mysql.connector.Error as e:
            logger.error('Database error: %s' % e)
        except Exception as ex:
    ...
    def upsert(self, tableName, valueDict, keyDict):
        ...
        try:
            c = self.getCursor()
            fdq = "SELECT COUNT(*) FROM "+tableName+" WHERE " + " AND ".join(genParams(keyDict))
            self.query(c,fdq,keyDict.values())
            fd = c.fetchone()[0]
            if fd == 0:
                # insert
                sql = "INSERT INTO "+tableName+" ( " + ",".join(valueDict.keys() + keyDict.keys()) + " )"
                sql = sql + " VALUES ( " + ", ".join("?" * len(valueDict.keys() + keyDict.keys())) + " )"
                self.query(c,sql,valueDict.values() + keyDict.values())
            else:
                # update 
                sql = "UPDATE "+tableName+" SET " + ",".join(genParams(valueDict))
                sql = sql + " WHERE " + " AND ".join(genParams(keyDict))
                self.query(c,sql,valueDict.values() + keyDict.values())
            c.close()
            self.connection.commit()
        except mysql.connector.Error as e:
